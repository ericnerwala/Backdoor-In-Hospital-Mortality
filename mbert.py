...
FTmodel = torch.load('/data/eric/CSIRE/model/mmtransformer/Checkpoints/BioClinicalBERT_FT',  map_location=torch.device('cpu')) 
FTmodel.eval()
...
def Read_Aggregate_data(mode, AggeragetNotesStrategies, discretizer=None, normalizer = None):
    ...
    dataPath = os.path.join('/data/eric/CSIRE/Data', mode + '_data_' +  File_AggeragetNotesStrategies + '.pkl')
    ...
    tokenized_path = os.path.join('/data/eric/CSIRE/Data', EmbedModelName + '_tokenized_ids_attns_' + mode + '_' + File_AggeragetNotesStrategies + '_' + str(MaxLen) + '_truncate.pkl')
