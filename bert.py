...
from mimic3_benchmarks.mimic3models.in_hospital_mortality import utils as ihm_utils
...
optimizer = AdamW(Ftmodel.parameters(), lr=2e-5)
criterion = torch.nn.CrossEntropyLoss()

# Define the training loop
def train_model(model, train_data, val_data, optimizer, criterion, device, num_epochs=10):
    model.train()
    model.to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_data:
            optimizer.zero_grad()
            float_data = []
            for sublist in batch[0]:
                float_sublist = []
                for element in sublist:
                    try:
                        float_element = float(element)
                        float_sublist.append(float_element)
                    except ValueError:
                        float_sublist.append(0.0)
                float_data.append(float_sublist)
            float_data = np.array(float_data)
            X = torch.tensor(float_data, dtype=torch.float).to(device)
            y = torch.tensor(batch[1], dtype=torch.float).to(device)
            text = [torch.tensor(x, dtype=torch.long).to(device) for x in batch[2]]
            attn = [torch.tensor(x, dtype=torch.long).to(device) for x in batch[3]]
            times = [torch.tensor(x, dtype=torch.float).to(device) for x in batch[4]] ## change from long to float

            print(text.shape)
            # print(attn)
            # print(times)
            
            logits, _ = model(**text) ## deleted attn, times
        ...
        with torch.no_grad():
            for batch in val_data:
                X = torch.tensor(batch[0], dtype=torch.float).to(device)
                y = torch.tensor(batch[1], dtype=torch.float).to(device)
                text = [torch.tensor(x, dtype=torch.long).to(device) for x in batch[2]]
                attn = [torch.tensor(x, dtype=torch.long).to(device) for x in batch[3]]
                times = [torch.tensor(x, dtype=torch.long).to(device) for x in batch[4]]
                
                logits, _ = model(text, attn, times)
                loss = criterion(logits, y)
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "/data/eric/CSIRE/model/best_model.pt")
...
def concat_text_timeseries(data_reader, data_raw):
    ...
    train_text, train_times, start_time = data_reader.read_all_text_append_json(data_raw['names'], 48, NumOfNotes=NumOfNotes, notes_aggeregate = 'Mean')
...
def Read_Aggregate_data(mode, AggeragetNotesStrategies, discretizer=None, normalizer = None):
    ...
    dataPath = os.path.join('/data/eric/CSIRE/model', mode + '_data_' +  File_AggeragetNotesStrategies + '.pkl')
    if os.path.isfile(dataPath):
        ...
        reader = InHospitalMortalityReader(dataset_dir=ReaderPath,
                                                  listfile=os.path.join(conf.ihm_path, mode + '/listfile.csv'), period_length=48.0)
    ...
    tokenized_path = os.path.join('/data/eric/CSIRE/model', EmbedModelName + '_tokenized_ids_attns_' + mode + '_' + File_AggeragetNotesStrategies + '_' + str(MaxLen) + '_truncate.pkl')
...
def train(model, data_X_val, data_y_val, data_text_val, txt_ids_eval, attn_val, times_val, names_eval, batch_size):
    # print(data_X_val.shape)
    # print(data_y_val.shape)
    # print(data_X_val[0])
    # print(data_y_val[0])
    train_batches = generate_padded_batches(data_X_val, data_y_val, data_text_val, txt_ids_eval, attn_val, times_val, batch_size)
    prf_val, probablities = train_model(model, train_batches, names_eval, optimizer, criterion, device)


train(Ftmodel, data_X, data_y, data_text, txt_ids, attention_masks, data_times, data_names, batch_size)
model_path = '/data/eric/CSIRE/model/best_model.pt'
tuned_model_state_dict = torch.load(model_path)
# Load the state dict into the model
NEWmodel = torch.load_state_dict(tuned_model_state_dict)
validate(NEWmodel, test_data_X, test_data_y, test_data_text, txt_ids_test, attention_masks_test, test_data_times, test_data_names, batch_size)
