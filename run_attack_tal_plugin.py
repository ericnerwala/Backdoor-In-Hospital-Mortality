...
if __name__=='__main__':
    ...
    if args.attacked_bs_name == "badnets" or args.attacked_bs_name == "ripples" or args.attacked_bs_name == "ep": # for badnet, the trigger should be list
    ...
    if config['attacker']['poisoner']['name'] == 'attn':
        config['attacker']['train']['save_path'] = './models_zoo_attn/{}_{}'.format(args.architectures, args.tasks)
        model_root = os.path.join(config['attacker']['train']['save_path'], f'{poison_setting}-{poisoner}-{args.attacked_bs_name}-{args.dataset_name}-{poison_rate}-attn{args.attn_distribute}-head{args.attn_head_num}', str(args.model_folder))
