...
def main(config):
    # set up logger file
    logger = init_logger(log_file = os.path.join(config['attacker']['train']['model_root'],'pos_to_neg2','log.txt') )
    ...
    backdoored_model, train_results = attacker.attack(victim, poison_dataset, 'pos_to_neg', config) 
    ...
    results = attacker.eval(backdoored_model, target_dataset, 'pos_to_neg')
